...
function summaryDetails(item:DirectoryItem,basePath:string){
 ...
 if(basePath.includes("workflows")){
  return [`Pemicu: ${item.trigger || "Saat input baru masuk"}`,`Estimasi setup: ${item.estimatedSetup || item.tag}`,`Output: ${(item.outputs || []).join(", ") || item.details[2]}`];
 }
 if(basePath.includes("tools")){
  return [`Cocok untuk: ${(item.bestFor || [item.description]).join(", ")}`,`Harga: ${item.pricingLabel || item.tag}`,`Terverifikasi: ${item.lastVerified || "Belum diset"}`];
 }
