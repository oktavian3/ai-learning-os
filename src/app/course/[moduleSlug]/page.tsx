...
export default async function ModulePage({params}:{params:Promise<{moduleSlug:string}>}){
  ...
  return <>
    ...
    <section className="section section-dark"><div className="container"><div className="grid grid-3"><div className="glass card"><h3>Paham konsepnya</h3><p>Secukupnya. Agar tidak asal menekan tombol dan bingung pas hasilnya aneh.</p></div><div className="glass card"><h3>Langsung coba</h3><p>Pakai prompt dan latihan kecil. Tidak perlu nunggu merasa jago.</p></div><div className="glass card"><h3>Simpan buktinya</h3><p>Hasil latihan dan project bisa jadi bahan portfolio—asal didokumentasikan.</p></div></div>{nextModule&&<Link href={`/course/${nextModule.slug}`} className="btn btn-primary" style={{marginTop:30}}>Lanjut ke: {nextModule.title}<ArrowRight size={15}/></Link>}</div></section>
  </>;
}
