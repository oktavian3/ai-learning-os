...
export default async function LessonPage({params}:{params:Promise<{moduleSlug:string;lessonSlug:string}>}){
  ...
  return <>
    ...
    <section className="section"><div className="container prose">
      <div className="glass card"><div className="card-top"><span>SELESAI BELAJAR INI, KAMU BISA</span><span>{lessonIndex+1}/{module.lessons.length}</span></div><ul className="list">{lesson.objectives.map(item=><li key={item}>{item}</li>)}</ul></div>
      ...
      {lesson.quiz.length>0&&<><h2>Cek pemahamanmu</h2><p>Jawab dengan bahasamu sendiri. Kalau masih susah menjelaskannya, baca lagi bagian penjelasan.</p><div className="glass card"><ol className="steps">{lesson.quiz.map((question,index)=><li key={question}><span>{index+1}</span><p>{question}</p></li>)}</ol></div></>}

      {lesson.assignments.length>0&&<><h2>Tugas kecil</h2>{lesson.assignments.map(item=><div className="callout" key={item}>{item}</div>)}</>}
