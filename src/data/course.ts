...
import {lessonContent,preserveAcronyms} from "./lessonContent";
...
const casualize=(text:string)=>preserveAcronyms(text
...
const pick = (codes:string[], difficulty:Difficulty):Lesson[] => codes.map(code => {
  ...
  return {
    ...
    content: source.content.map(casualize),
    examples: source.examples.map(casualize),
    exercises: source.exercises.map(casualize),
    prompts: source.prompts.map(casualize),
    quiz: source.quiz.map(casualize),
    assignments: source.assignments.map(casualize),
    notes: source.notes.map(casualize),
...
export const courseModules:CourseModule[] = [
  ...
  module(2,"generative-ai-llm","Generative AI & LLM","Bongkar cara LLM kerja, apa itu token dan context window, plus cara pilih model tanpa hafal semua merek.","Pemula","3 jam",["Mengerti cara LLM menghasilkan jawaban","Bisa memilih model sesuai tugas"],["ChatGPT","Claude","Gemini"],"Demo 1 input jadi 5 output",["M01.L02","M01.L03","M02.L01","M02.L02","M02.L03"]),
